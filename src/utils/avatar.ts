...
// Draw background: deep dark + scanline grid
function drawBackground(ctx: CanvasRenderingContext2D, w: number, h: number, accentColor: string) {
  ...
  // Scanlines (subtle horizontal stripes)
...
// Draw stylized face (geometric, cyberpunk aesthetic)
function drawFace(
  ...
  // Face silhouette — slightly angular
  ctx.beginPath();
  ...
  // Jaw angular cutout (makes it more geometric)
  ...
  // Mouth — thin line with slight curve
...
// Hair — colored geometric shapes on top of head
function drawHair(
  ctx: CanvasRenderingContext2D,
  cx: number,
  cy: number,
  ...
  if (hairStyle === 0) {
    // Mohawk — tall rectangle
    ctx.beginPath();
    ctx.rect(cx - 12, topY - 40, 24, 45);
    ctx.fill();
    ctx.stroke();
  } else if (hairStyle === 1) {
    // Side-swept — parallelogram
    ctx.beginPath();
    ctx.moveTo(cx - 50, topY + 5);
    ctx.lineTo(cx + 30, topY + 5);
    ctx.lineTo(cx + 15, topY - 30);
    ctx.lineTo(cx - 65, topY - 30);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
  } else if (hairStyle === 2) {
    ...
    for (let i = 0; i < spikes; i++) {
      ...
      ctx.lineTo(sx + 11, topY + 5);
      ctx.lineTo(sx + 5.5, topY - h);
      ctx.closePath();
      ctx.fill();
      ctx.stroke();
    }
  } else if (hairStyle === 3) {
    // Long and geometric — trapezoid
    ctx.beginPath();
    ctx.moveTo(cx - 55, topY + 10);
    ctx.lineTo(cx + 55, topY + 10);
    ctx.lineTo(cx + 40, topY - 35);
    ctx.lineTo(cx - 40, topY - 35);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
  } else {
    // Undercut — flat top box
    ctx.beginPath();
    ctx.rect(cx - 45, topY - 20, 90, 25);
    ctx.fill();
    ctx.stroke();
  }
...
// Cybernetic implants — circuit lines, augments drawn on face/head
function drawCyberware(
  ...
  for (let i = 0; i < implantCount; i++) {
    ...
    if (implantType === 0) {
      // Temple implant — lines from temple outward
      const tx = cx + side * 40;
      const ty = cy - 20;
      ctx.beginPath();
      ctx.moveTo(tx, ty);
      ctx.lineTo(tx + side * 25, ty - 10);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(tx, ty + 5);
      ctx.lineTo(tx + side * 20, ty + 10);
      ctx.stroke();
      // Node dot
      ctx.beginPath();
      ctx.arc(tx + side * 25, ty - 10, 2.5, 0, Math.PI * 2);
      ...
      ctx.lineTo(startX + side * 15, startY);
      ctx.lineTo(startX + side * 15, startY + 15);
      ctx.lineTo(startX + side * 25, startY + 15);
      ctx.stroke();
      // Junction dots
      [0, 15].forEach((offset) => {
        ctx.beginPath();
      ...
      // Forehead implant bar
      ctx.beginPath();
      ctx.moveTo(cx + side * 10, cy - 40);
      ctx.lineTo(cx + side * 38, cy - 40);
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(cx + side * 38, cy - 40, 3, 0, Math.PI * 2);
      ctx.fillStyle = accentColor;
      ctx.fill();
      ...
      const jy = cy + 38;
      ctx.beginPath();
      ctx.moveTo(cx + side * 12, jy);
      ctx.lineTo(cx + side * 35, jy + 5);
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(cx + side * 35, jy + 5, 2, 0, Math.PI * 2);
      ctx.fillStyle = secondaryColor;
      ctx.fill();
    }
