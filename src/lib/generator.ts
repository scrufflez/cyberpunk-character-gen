...
import { backstoryTemplates } from './backstories';
import { roles, roleStatBias, roleAbilities } from './classes';
...
export function calculateDerivedStats(stats: Stats): DerivedStats {
  ...
  return { hitPoints, humanity, seriouslyWounded, deathSave, initiative };
}

function generateRoleAbility(role: RoleClass): RoleAbility {
  return { name: roleAbilities[role], rank: 4 };
}
